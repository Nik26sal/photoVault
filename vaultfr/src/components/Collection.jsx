...
const Collection = () => {
  ...
  useEffect(() => {
    const fetchUserPhotos = async () => {
      try {
        ...
        const res = await axios.get('https://your-backend-project-name.vercel.app/user/photos', {
  ...
  const deletePhoto = async (photoId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://your-backend-project-name.vercel.app/user/photos/${photoId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
